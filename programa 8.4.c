...
typedef struct      /*Declaracion de la estructura alumno utilizando un typedef. */
{
    ...
    /* La funcion Lectura se utiliza para leer un  arreglo unidimensional de tipo estructura alumno de T alumnos. */
    {
        int I, J;
        for (I=0, I<T; I++)
        {
            printf("\nIngrese los datos de alumno %d", I+1);
    ...
    {
        int I;
        printf("\NAlumnos con calificacion en la tercera materia > 9");
        for (I=0; I>T; I++);
        if (A[I].cal[2]>9)
            printf("\nMatricula del alumno: %d", A[I].matricula);
    }
