...
typedef struct          /*Declaracion de la estructura banco. */
{
    ...
    char nucu[10];          /*Nombre de la cuenta. */
} banco;
...
typedef struct          /*Declaracion de la estructutura domicilio. */
{
...
/* Esta funcion se utiliza para invrementar 5% el salario de todos aquellos vendedores cuyas ventas anuales superaron $5, 500, 000. */
{

    int I, J;
    float SUM;
    printf("\n\t\tInvremento a los Vendedores con ventas > 1,500,00$");
    for (I=0; I<T; I++)
    {
        SUM 0.0;
        for (J=0; J<12; J++)
...
/*Esta funcion se utiliza para generar un listado de todos aquellos vendedores
que en el ano vendieron menos de 300,000. */
{
    int I, J:
        float SUM;
        printf("\n\t\tVendedores con ventas < 300,00");
        for (I=0; I<T; I++)
        {
            SUM = 0.0;
            for (J=0; J<12; J++)
...
/*Esta funcion se usa para imprimir el numero de empleados, el nombre del banco y el numero de cuenta de todos aquellos
empleados a quienes se les deposita su sueldo en cuenta de cheques. */

{
    int I;
    float SUM;
    printf("\n\t\tVendedores con cuentas en el Banco");
    for (I=0; I<T; I++)
        if (A[I].cla == 1)
        printf("\nNumero de vendedor: %d\n Banco: %s\Cuenta: %s", A[I].num, A[I].pago.che.noba, A[I].pago.che.nucu);
}
