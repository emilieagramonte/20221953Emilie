...
typedef struct              /*Declaracion de la estructura popropiedades. */
{
    char clave[5];
    float scu;                  /*Superficie cubierta. */
    float ste;                  /* Superficie terreno. */
    char ca[50];                /* Caracteristicas. */
    ubicacion ubi;              /*Observa que este campo es de tipo
...
void main (void)
{
    ...
    do
    {
     ...
     while (TAM > 100 || TAM < 1);
     /*Se verifica que el tamano del arreflo sea correcto. */
     Lectura(PROPIE, TAM);
     F1(PROPIE, TAM);
     F2(PROPIE, TAM);
     }
     ...
     propiedades de T elementos.*/
     {
      ...
      /*Esta funcion se utiliza para generar un listado de las propiedades disponibles para venta en la zona de miraflores,
...

      {
        ...
        for (I=0; I<T; I++)
            ...
            if ((A[I].precio >= 45000) && (A[I].precio <= 65000))
        {
            printf("\nClave  de la propiedad: ");
            puts(A[I].clave);
            printf("\nSuperficie cubierta: %f", A[I].scu);
            printf("\nSuperficie terrreno: %f", A[I].ste);
            printf("\nCaracteristicas: ");
            puts(A[I].car)
            printf("Calle: ");
            puts(A[I].ubi.calle);
