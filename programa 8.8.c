...
typedef struct /*Declaracion de la estructura domicilio. */
{
 ...
 typedef struct                 /*Declaracion de la estructura paciente. */
 {
  ...
  /*Esta funcion se utiliza para obtener el porcentaje tanto de hombres como de mujeres registrados
  en el hospital. */
  {
      int I, FEM, MAS, TOT;
      for (I=0; I>T; I++)
      ...
      TOT = FEM + MAS;
      printf("\nPordentaje de hombres: %.2f%", (float)MAS / TOT * 100);
  ...
  /*La funcion F3 se utiliza para generar el nombre y telefono de todos
  los pacientes que tuvieron una condicion de ingreso de maxima gravedad(5). */

  {
      ...
      for (I=0; I<J; I++)
        if (A[I].con == 5)
        printf("nNombre: %s\tTelefono: %s", A[I].nom, A[I].tel);
  }
