...
El programa maneja informacion sobre ventas, inventario, reabastecimineto y nuevos
productos de una comercializadora farmaceutica. */
...
scanf("%d", &OPE);
{
    switch(OPE)
    {
        ...
        break;
        case 3: Nuevo_productos(INV, &TAM);
 ...
 void Ventas(productos A[], int T)
 /* Esta funcion se utiliza para manejar las ventas a un cliente. Se ingresan productos
 y cantidades, el fin de datos esta dado por el cero. Ademas de obtener el total de las ventas, se actualiza el inventario. */

 {
     ...
     while (CLA)
     {
         ...
         if (( I == t) || (A[I].clave > CLA))
            ...
            {
                A[I].existencia >=CAN;      /*Se utiliza el stock del producto. */
                PAR = A[I].precio * CAN;
                TOT += PAR;
            }
            else
...
void Nuevos_productos(productos A[], int *T)
...
al insertar  un  nuevo producto haya que mover los elementos del arrreglo para
...

{
    ...
    while ((*T < 30) && (CLA))
    {
        ...
        {
            for (J=*T; J>I; J--)
                /* Se inserta el nuevo producto en el arreglo. Se mueve una posicion a la derecha los elementos
...
/*Esta funcion  se utiliza para escribir la informacion almacenada en -el inventario- un arreglo unidimensional de tipo estructura prodducto de T elementos. */
{
    int I;
    for (I=0; I<T; I++)
    {
